	...
	function Timer(){
		...
		function construct(time, interval, state, bottom, top, loop, speed, withEvents){
			...
			if( typeof(time) == "boolean" ){
				withEvents = time;
				time = SH_TIMER_DEFAULT_TIME;
			}else if( typeof(interval) == "boolean" ){
				withEvents = interval;
				interval = SH_TIMER_DEFAULT_INTERVAL;
			}else if( typeof(state) == "boolean" ){
				withEvents = state;
				state = SH_TIMER_DEFAULT_PLAYSTATE;
			}else if( typeof(bottom) == "boolean" ){
				withEvents = bottom;
				bottom = SH_TIMER_DEFAULT_BOTTOM_CAP;
			}else if( typeof(speed) == "boolean" ){
				withEvents = speed;
				speed = SH_TIMER_DEFAULT_SPEED;
			}
			...
			me.bottom      = (typeof(bottom)     === "number")?  bottom     : SH_TIMER_DEFAULT_BOTTOM_CAP;       _reset.bottom   = me.bottom;
			me.loop        = (typeof(loop)       === "boolean")? loop       : SH_TIMER_DEFAULT_LOOP;             _reset.loop     = me.loop;
			me.speed       = (typeof(speed)      === "number")?  speed      : SH_TIMER_DEFAULT_SPEED;            _reset.speed    = me.speed;
			me.interval    = (typeof(interval)   === "number")?  interval   : SH_TIMER_DEFAULT_INTERVAL;         _reset.interval = me.interval;
			me.time        = (typeof(time)       === "number")?  time       : SH_TIMER_DEFAULT_TIME;             _reset.time     = me.time;
			me.top         = (typeof(top)        === "number")?  top        : SH_TIMER_DEFAULT_TOP_CAP;          _reset.top      = me.top;
			
			_state         = (typeof(state)      === "number")?  state      : SH_TIMER_DEFAULT_PLAYSTATE;        _reset.state    = _state;
			_withEvents    = (typeof(withEvents) === "boolean")? withEvents : SH_TIMER_DEFAULT_RAISES_EVENTS;
			
			if( me.state == SH_TIMER_STATE_PLAYING ){
				play();
			}
		}
...
/**[ Public Properties ]******************************************************[Serious Hare]**/
	 ...
	 *****************************************************************************[Serious Hare]**/
		Object.defineProperty(me, "bottom", {
			...
			set:function(value){
				_bottom = (typeof value === "number")? toZero(value) : SH_TIMER_DEFAULT_BOTTOM_CAP;
			},
			enumerable:true,
		})
		var _bottom = SH_TIMER_DEFAULT_BOTTOM_CAP;
	 ...
	 *****************************************************************************[Serious Hare]**/
		Object.defineProperty(me, "speed", {
			...
			set:function(value){
				...
				baseTime = toZero(me.time);
				// set the startTime to the current time
				startTime = Date.now();
				
				_speed = value? value : SH_TIMER_DEFAULT_SPEED;
			},
			enumerable:true,
		})
		var _speed = SH_TIMER_DEFAULT_SPEED;
	 ...
	 *****************************************************************************[Serious Hare]**/
		Object.defineProperty(me, "top", {
			...
			set:function(value){
				_top = value? toZero(value) : SH_TIMER_DEFAULT_TOP_CAP;
			},
			enumerable:true,
		})
		var _top = SH_TIMER_DEFAULT_TOP_CAP;
	 ...
	 *****************************************************************************[Serious Hare]**/
		Object.defineProperty(me, "interval", {
			...
			set:function(value){
				_interval = value? Math.max(1, value) : SH_TIMER_DEFAULT_INTERVAL;
			},
			enumerable:true,
		})
		var _interval = SH_TIMER_DEFAULT_INTERVAL;
	 ...
	 *****************************************************************************[Serious Hare]**/
		Object.defineProperty(me, "time", {
			get:function(){
				...
				var returnTime = toZero( baseTime + (( Date.now() - startTime ) * ( _speed * Math.min(_state, 1) )));
...
/**[ Public Control Methods ]*************************************************[Serious Hare]**/
	 ...
	 *****************************************************************************[Serious Hare]**/
		function addMarker(time, callback){
			...
			var marker = new TimerMarker(time, callback)
...
/**[ Private Control Methods ]************************************************[Serious Hare]**/
	 ...
	 *****************************************************************************[Serious Hare]**/
		function setInterval(){
			// Create an interval and attach the tick handler if there is no timer running.
			if(interval == null && _withEvents) interval = window.setInterval(update, SH_TIMER_PRECISION);
		}
	 ...
	 *****************************************************************************[Serious Hare]**/
		...
		function testTick(workTime, fireOnce){
			var prevTickTime = prevTime % me.interval;
			var tickTime = me.time % me.interval;
			
			if(me.speed > 0 && _state > 0){
				if(tickTime < prevTickTime){
					dispatchEvent(new TimerEvent("tick", me, workTime));
				}
			}else if(me.speed < 0 && _state > 0){
				if(tickTime > prevTickTime){
					dispatchEvent(new TimerEvent("tick", me, workTime));
				}
			}
		...
		function testMarkers(workTime, fireOnce){
			for(var i=0; i<markers.length; i++){
				...
				if(me.speed > 0 && (_state > 0 || fireOnce) && marker){
				...
				}else if(me.speed < 0 && (_state > 0 || fireOnce) && marker){
					if(((prevTime > marker.time) && (workTime < marker.time)) || (workTime == marker.time)){
						var event = new TimerEvent("marker", marker);
						if(marker.callback){
							marker.callback(event);
...
/**[ Event Methods/Functions ]************************************************[Serious Hare]**/
	 ...
	 *****************************************************************************[Serious Hare]**/
		function dispatchEvent(event){
			...
			if(stack.on)stack.on.call(this, event);
	 ...
	 *****************************************************************************[Serious Hare]**/
		...
		Object.defineProperty(me, 'onpause',    {get: function(){return listeners.pause.on;},  set: function(value){listeners.pause.on = value;},  enumerable:true});
	...
	Object.defineProperty(window, "Timer", {value:Timer, enumerable:true});
